...
If you've talked to me for 5 minutes, you know that obfuscation is not the way to go, so how do you protect content in the [man at the end situation]() ?
Well, technically we aren't in a man-at-the-end situation, if you're reading this it means that I trust you, that someone betrayed my trust or that this is all broken.
...
Each user is sent a one-time URL with a 256-bit AES key (generated from 32 random bytes, base64url-encoded).
...
Each post lives at `/posts/<slug>.enc`. Slugs are random 12-character hex strings — they are not derived from the post filename or title. This prevents guessing post URLs.
...
1. You receive a one-time link: `https://example.com/login#key=<base64url-key>&uid=<16-char-hex>`
2. The `/login/` page loads `auth.js`, which parses the fragment, sets `__Host-key` and `__Host-uid` cookies (Secure, SameSite=Lax, 1 year, path=/), then redirects to `/`.
3. The root page (`/`) loads `index.js`, which reads the cookies, fetches `/users/<uid>/index.enc`, decrypts it with your key, and injects the HTML — showing a greeting and a list of posts you can access.
...
- Cross-device support for v1 (new device = need the one-time link again).
...
| `/logout/` | Clears key and uid cookies, redirects to `/` |
...
- Prints the one-time link (`/login#key=...&uid=...`).
...
- For each post: generates a random 12-char hex slug, generates a new random DEK, encrypts the HTML.
...
Their old `.enc` manifest and index are no longer deployed after the next rebuild. They can't read new posts.
...
```
default-src 'none'; script-src 'self'; style-src 'self'; connect-src 'self'; img-src 'self' data:; base-uri 'none'
```
